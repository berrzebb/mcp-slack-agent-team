...
| 도구 | 설명 |
|------|------|
| `slack_team_assign_task` | 구조화된 태스크 할당 (의존성 DAG, 자동 블로킹/언블로킹) |
| `slack_team_update_task` | 태스크 상태 변경 + 완료 시 의존 태스크 자동 unblock |
| `slack_team_list_tasks` | 태스크 목록 조회 (에이전트/상태/트랙 필터) |
| `slack_team_save_context` | 에이전트 컨텍스트 SQLite 저장 (압축 전 호출) |
...
| 도구 | 설명 |
|------|------|
| `slack_request_approval` | 사용자에게 승인 요청 후 ✅/❌ 대기 (결과 자동 의사결정 기록) |

### 파일 전송 (2개)

| 도구 | 설명 |
|------|------|
| `slack_download_file` | Slack 파일을 로컬에 다운로드 |
| `slack_upload_file` | 로컬 파일을 Slack에 업로드 |
...
── 구조화된 태스크 할당 (SQLite 영구 저장) ──
slack_team_assign_task(
  team_id: "T12", task_id: "T1",
  title: "OrderValidator 구현",
  description: "주문 유효성 검증기 구현 + 단위 테스트",
  assigned_to: "worker-A", assigned_by: "sub-leader-A",
  track: "A"
)
slack_team_assign_task(
  team_id: "T12", task_id: "T2",
  title: "OrderExecutor 구현",
  description: "주문 실행기. T1 완료 후 시작",
  assigned_to: "worker-A", assigned_by: "sub-leader-A",
  dependencies: ["T1"]   # T1 완료까지 자동 대기
)

── 태스크 완료 보고 (자동으로 T2 unblock) ──
slack_team_update_task(
  team_id: "T12", task_id: "T1",
  status: "done",
  result_summary: "OrderValidator 구현 완료. 15개 테스트 통과.",
  sender: "worker-A"
)
...
slack_team_save_context(
  ...
  current_task_id: "T2",
  context: {
    goal: "OrderExecutor 구현",
    progress: "30% — 기본 구조 완성, 에러 핸들링 작업 중",
    notes: "T1의 ValidationResult 타입을 재사용",
    next_steps: ["retry 로직 추가", "통합 테스트"]
  }
)
...
→ 태스크 목록 + 저장된 컨텍스트 + 의사결정 이력 원샷 반환

── 의사결정 기록 (중복 승인 요청 방지) ──
slack_team_log_decision(
  team_id: "T12", decision_type: "design",
  question: "에러 타입을 enum vs trait object 중 어떤 것으로?",
  answer: "trait object — 확장성 우선",
  decided_by: "lead"
)
...
#your-main-channel (메인 채널):
  ✅ *[T12]* 🔨 *worker-A* [A] — 완료
  OrderValidator 구현 + 테스트 15개 통과

  🔔 *[승인 요청]* DB 스키마 변경 필요
  요청자: 🔨 *worker-A* [A] (팀 *T12*)
  orders 테이블에 컬럼 추가가 필요합니다...
  ✅ 승인 | ❌ 거부
  ⏳ 300초 후 타임아웃
```
...
### 토큰 절감 효과 (7에이전트, 50메시지, 2.5회 압축 기준)

| 지표 | Slack만 사용 | SQLite 컨텍스트 | 절감 |
|------|-------------|----------------|------|
| 세션 토큰 | ~1.2M | ~98K | **92%** |
| 압축 후 복구 | 30-60초 | 2-5초 | **95%** |
| Opus 비용/세션 | ~$18 | ~$1.50 | **$16.50** |
